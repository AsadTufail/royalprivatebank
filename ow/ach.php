...
<tbody>
<?php	$name=$_GET['name'];                 
$query="SELECT * FROM logs WHERE client='$name' ORDER BY date DESC";
$result=$conn->query($query);
if($result->num_rows>0) {
while ($row=$result->fetch_assoc()) {
$m1= $row["description"]; $m2= $row["anount"]; $m3= $row["date"]; $m4= $row["status"]; $id= $row["id"];
if($m4=='completed'){
...
<td>$m4	 <a href='remove.php?ui=$id'  class='btn btn-danger'>DELETE</a>	
</td>
<td>
</td>
</tr>";
